...
发布到 GitHub 后，可以通过下面的命令安装：

```bash
git clone https://github.com/YLTsing/ai-evaluation-skill.git \
  "${CODEX_HOME:-$HOME/.codex}/skills/ai-evaluation-skill"
```

重新启动 Codex 或开始一个新会话，使其重新发现 Skill。也可以把整个仓库放入其他 Agent 支持的 skills 目录，或直接在当前仓库中作为项目内 Skill 使用。
